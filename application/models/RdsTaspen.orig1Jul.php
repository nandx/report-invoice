...
class RdsTaspen extends CI_Model
{
	...
	public function mlookupfilterall()
	{
		...
		if ($cari == 'all') {
			...
			$data = $this->db->query("SELECT * from tl_invoice_standard where  ID_CHILD != 27 AND IDDIVISION IS NOT NULL AND IDSUB IS NOT NULL AND IDSUB > 0 order by DUEDATE ASC");
		}
		return $data->result();
	...
	{
		$data = $this->db->query("SELECT * from tl_invoice_standard where ID_CHILD != 27 AND IDDIVISION IS NOT NULL  AND IDSUB > 0 order by DUEDATE ASC");
		return $data->result();
	}
		...
		$data = $this->db->query("SELECT *,
				CASE 
					WHEN (LENGTH(MONTH(DUEDATE)) < 2)
 					...
 					ELSE MONTH(DUEDATE)
				END AS DUEDATEMONTH 
			FROM tl_invoice_standard where id = '$id' AND ID_CHILD !=27  order by `DUEDATE` ASC");
		return $data->result();
	}
	public function sumjmlpremi($id)
	{
		$data = $this->db->query("SELECT sum(jmlpremi) as  jml from tl_invoice_standard where id = '$id' AND ID_CHILD !=27 order by `DUEDATE` ASC");
		return $data->result();
	}
	...
	{
		//$cari = $this->input->GET('cari', TRUE);
		$data = $this->db->query("SELECT * from tl_invoice_standard where ID_CHILD = 27 order by `DUEDATE` ASC");
		return $data->result();
	}
	...
	public function mlookupaspur()
	{
		...
		if (!empty($cari) and !empty($year)) {
			$data = $this->db->query("SELECT * from tl_invoice_standard HAVING MONTH(DUEDATE) =  '$cari' AND YEAR(DUEDATE) ='$year' AND ID_CHILD = 27 AND NOINVOICE IS NOT NULL and KOTA IS NOT NULL order by `DUEDATE`  ASC");
		} else {
			$data = $this->db->query("SELECT * from tl_invoice_standard where ID_CHILD = 27 AND NOINVOICE IS NOT NULL and KOTA IS NOT NULL order by `DUEDATE` ASC");
		}
		return $data->result();
	}
	...
	{
		$data = $this->db->query("SELECT sum(premi) as  jml from aspurjabnew where id = '$id'  order by `DUEDATE` ASC ");
		return $data->result();
	}

	public function premias($id, $bulan, $tahun)
	{
		$data = $this->db->query("SELECT productname , jmlpst, premi from aspurjabnew where BULAN='$bulan' and TAHUN='$tahun' AND id = '$id'  order by `DUEDATE` ASC");
		return $data->result();
	}

	public function querywhereasloop($id, $bulan, $tahun)
	{
		$data = $this->db->query("SELECT DISTINCT CREATEDATE, DUEDATE, PAYMENTDATE, TAHUN, BULAN, PARTNERNAME, NMDIVISION, NMSUB, ALAMAT, KOTA, NOINVOICE, POLICYNO, JMLPREMI, JMLPST, TERBILANG, NAME, PREMI, CURRENCY, PRODUCTCODE, PRODUCTNAME, BANKNAME, ACCOUNTNAME, ACCOUNTNUMBER, STATUS, REV FROM aspurjabnew where BULAN='$bulan' and TAHUN='$tahun' AND id = '$id'  order by `DUEDATE` ASC");
		return $data->result();
	}
		...
		$data = $this->db->query("SELECT *,
				CASE 
					WHEN (LENGTH(MONTH(DUEDATE)) < 2)
 					...
 					ELSE MONTH(DUEDATE)
				END AS DUEDATEMONTH 
 				FROM tl_invoice_standard where id = '$id' order by `DUEDATE` ASC");
		return $data->result();
	}

	public function sumpremias($id, $bulan, $tahun)
	{
		$data = $this->db->query("SELECT sum(premi) as  jml from aspurjabnew where BULAN='$bulan' and TAHUN='$tahun' AND id = '$id'  order by `DUEDATE` ASC");
		return $data->result();
	}
